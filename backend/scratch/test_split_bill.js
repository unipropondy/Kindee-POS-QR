...
async function run() {
  ...
  // 3. Process Part 1 (Split 3 parts: Qty 0.333 of each)
  console.log("💳 Paying Part 1 (0.333 Qty)...");
  const part1Res = await fetch(`${API_URL}/api/sales/save`, {
    ...
    body: JSON.stringify({
      ...
      isSplit: "true",
      paymentMethod: "CASH",
      totalAmount: 2.16, // (0.333 * 1.00 + 0.333 * 5.50) * 1.09 GST = 2.163
      subTotal: 1.98,
      items: [
        {
          lineItemId: "11111111-1111-1111-1111-111111111111",
      ...
      payments: [{ payMode: "CASH", amount: 2.16 }],
  ...
  // Sync cart for Part 2 (simulating client setContextItems with 0.667 remaining)
  console.log("🔄 Syncing remaining cart items to DB (0.667 remaining)...");
  ...
  // 4. Process Part 2 (Split 2 remaining parts: Qty 0.333 of each)
  console.log("💳 Paying Part 2 (0.333 Qty)...");
  const part2Res = await fetch(`${API_URL}/api/sales/save`, {
    ...
    body: JSON.stringify({
      ...
      payments: [{ payMode: "NETS", amount: 2.16 }],
      displayOrderId: orderNo,
      cashierId: "00000000-0000-0000-0000-000000000000"
    })
  });
  ...
  // Sync remaining cart for Part 3 (0.334 remaining)
  console.log("🔄 Syncing remaining cart items to DB (0.334 remaining)...");
  await fetch(`${API_URL}/api/orders/save-cart`, {
    method: "POST",
    headers: { 
      "Content-Type": "application/json",
  ...
  // 5. Process Part 3 (Final Part: Qty 0.334 of each, isSplit: false)
  console.log("💳 Paying Part 3 (0.334 Qty)...");
  const part3Res = await fetch(`${API_URL}/api/sales/save`, {
    ...
    body: JSON.stringify({
      ...
      isSplit: "false",
      paymentMethod: "CASH",
      totalAmount: 2.17, // (0.334 * 1.00 + 0.334 * 5.50) * 1.09 = 2.17
      subTotal: 1.99,
      items: [
        {
          lineItemId: "11111111-1111-1111-1111-111111111111",
      ...
      payments: [{ payMode: "CASH", amount: 2.17 }],
      displayOrderId: orderNo,
      cashierId: "00000000-0000-0000-0000-000000000000"
    })
  });
